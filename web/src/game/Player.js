...
export class Player extends Phaser.Physics.Arcade.Sprite {
    ...
    moveLeft() {
        ...
        this.body.setVelocityX(dir * MOVE_SPEED * this.speedMultiplier);
    ...
    stopHorizontal() { ; this.body.setVelocityX(0); }
    jump() {
        const body = this.body;
    ...
    applyTempEffect(type, ms) {
        if (type === 'fast') {
            this.speedMultiplier = 1.7;
            this.scene.time.delayedCall(ms, () => { if (this.active)
    ...
    updateAnimation() {
        ...
        if (this.hasInhaled)
            ...
            anim = 'float';
        else if (!body.blocked.down && body.velocity.y > 80)
            anim = 'fall';
    ...
    // ── inhale / spit ───────────────────────────────────────────────────────────
    setInhaling(active) { this.isInhaling = active; }
    ...
    capturePlayer(other) {
        if (this.inhaledObject || other.isInhaled || !other.isAlive)
            return;
        other.isInhaled = true;
        other.body.setEnable(false);
        other.setVisible(false);
        this.inhaledObject = other;
        this.isInhaling = false;
        this.setScale(1.3, 1.0);
        this.scene.time.delayedCall(PLAYER_CARRY_MS, () => this.spitObject());
    }
    spitObject() {
        ...
        this.setScale(1);
        if (obj.isInhaled !== undefined) {
            const other = obj;
            other.isInhaled = false;
            other.setVisible(true);
            other.setPosition(this.x + (this.flipX ? -50 : 50), this.y);
            other.body.setEnable(true);
            other.body.setVelocity(this.flipX ? -SPIT_VX : SPIT_VX, SPIT_VY);
        }
        else {
            obj.setVisible(true);
            obj.setPosition(this.x + (this.flipX ? -50 : 50), this.y);
            obj.body.setEnable(true);
            obj.body.setVelocity(this.flipX ? -SPIT_VX : SPIT_VX, SPIT_VY);
    ...
    swingRapierSprite() {
        if (!this.rapierSprite || this.isSwinging)
            return;
    ...
    hitByEnemy() {
        ...
        const dx = this.body.velocity.x > 0 ? -1 : 1;
        this.body.setVelocityX(dx * 250);
        if (this.hearts <= 0)
            this.die();
    }
    takeDamage(_amount, _type) { this.hitByEnemy(); }
    destroy(fromScene) {
