...
export class LobbyScene extends Phaser.Scene {
    ...
    create() {
        ...
        this.stars.push(this.add.tileSprite(width / 2, height / 2, width, height, 'bg-stars').setScrollFactor(0), this.add.tileSprite(width / 2, height / 2, width, height, 'bg-nebula-blue').setScrollFactor(0).setAlpha(0.6));
    ...
    buildJoining() {
        ...
        this.input.keyboard?.on('keydown', (event) => {
            if (!this.groups.get('joining')?.visible)
                return;
    ...
    async submitJoinCode(btn) {
        if (this.codeEntry.length < 6) {
            this.joinError.setText('Please enter 6 characters');
            return;
        }
        btn.setColor(DIM);
        this.joinError.setText('');
    ...
    getCodeLabel() {
        ...
        if (!grp)
            return undefined;
        return grp.list.find(obj => obj._isCodeLabel);
    }
