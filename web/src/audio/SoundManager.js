...
// ── waveform helpers ──────────────────────────────────────────────────────────
function tone(freq, t, dur, vol, type = 'triangle', dest) {
    const c = ac();
...
function noiseBlip(t, dur, vol, freq, q = 3) {
    const c = ac();
...
function scheduleWinLoop(start) {
    const c = ac();
    ...
    _winTimer = setTimeout(() => {
        if (_winPlaying)
            scheduleWinLoop(start + WIN_LOOP);
    }, Math.max(0, wait));
}
// ── public API ────────────────────────────────────────────────────────────────
